...
p_stock_total <- ggplot(core_totals, aes(x = reorder(stratum, -total_stock),
  ...
  labs(
    title = "Aerial Carbon Stock per Core",
    subtitle = sprintf("Depth-integrated stock per unit area (n = %d cores)", nrow(core_totals)),
    x = "Stratum",
    y = "Aerial Carbon Stock per Core (kg C/m²)",
    fill = "Stratum"
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
log_message("Copying key Module 02 outputs to outputs/Basic_analysis/Step1_NonSpatial/Plots...")
step1_plots_dir <- "outputs/Basic_analysis/Step1_NonSpatial/Plots"
dir.create(step1_plots_dir, recursive = TRUE, showWarnings = FALSE)

# Note: 01_spatial_distribution.png is excluded — it is a raw data QA map, not
# a non-spatial analysis output. It remains available in outputs/plots/exploratory/.
basic_module2_files <- c(
  "outputs/plots/exploratory/02_soc_distribution_by_stratum.png" = "SOC_Distribution_by_Stratum.png",
  "outputs/plots/exploratory/03_depth_profiles_by_stratum.png"   = "Depth_Profiles_by_Stratum.png",
  "outputs/plots/exploratory/05_carbon_stock_by_stratum.png"     = "Carbon_Stock_by_Stratum.png",
  "outputs/plots/exploratory/08_summary_table.png"               = "Summary_Table_of_Key_Statistics.png"
)

for (src in names(basic_module2_files)) {
  dst <- file.path(step1_plots_dir, basic_module2_files[[src]])
  if (file.exists(src)) {
    file.copy(src, dst, overwrite = TRUE)
    log_message(sprintf("Copied to Step1_NonSpatial/Plots: %s", basename(dst)))
  } else {
