...
# Presentation outputs — now nested under Step2 Kriging folder
dir.create("outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Maps",   recursive = TRUE, showWarnings = FALSE)
dir.create("outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Tables", recursive = TRUE, showWarnings = FALSE)
...
# ============================================================================
# LOAD AOI BOUNDARY (for masking kriging predictions to study area)
# ============================================================================
# The AOI is used to:
#   1. Clip all prediction rasters so they show only within-boundary values
#      (removes extrapolation outside the study area and white-gap artefacts)
#   2. Calculate spatial statistics (area, mean stock) consistent with
#      the simple extrapolation outputs in Step 2a

aoi_sf_proj  <- NULL    # projected AOI (sf), used for masking
aoi_vect_proj <- NULL   # terra SpatVector version of AOI

if (exists("AOI_FILE") && !is.null(AOI_FILE) && file.exists(AOI_FILE)) {
  tryCatch({
    aoi_raw      <- sf::st_read(AOI_FILE, quiet = TRUE)
    aoi_sf_proj  <- sf::st_transform(aoi_raw, PROCESSING_CRS)
    aoi_vect_proj <- terra::vect(aoi_sf_proj)
    log_message(sprintf("AOI loaded: %d feature(s), projected to EPSG:%d",
                        nrow(aoi_sf_proj), PROCESSING_CRS))
  }, error = function(e) {
    log_message(sprintf("Could not load AOI_FILE: %s — kriging will not be clipped to AOI", e$message), "WARNING")
  })
} else {
  log_message("AOI_FILE not set — kriging predictions will not be clipped to AOI boundary", "WARNING")
}

# Helper: mask a terra raster to the AOI and return the masked raster
mask_to_aoi <- function(rast_obj) {
  if (is.null(aoi_vect_proj)) return(rast_obj)
  tryCatch({
    r_crop  <- terra::crop(rast_obj, aoi_vect_proj)
    r_mask  <- terra::mask(r_crop,  aoi_vect_proj)
    r_mask
  }, error = function(e) {
    log_message(sprintf("AOI masking failed: %s — returning unmasked raster", e$message), "WARNING")
    rast_obj
  })
}
...
for (depth in STANDARD_DEPTHS) {
  ...
  for (stratum_name in strata) {
    ...
    # Masks the prediction and variance rasters to the AOI polygon so that
    # only within-boundary cells are retained. This removes white-gap artefacts
    # that appear when the kriging grid extends beyond the study area.
    pred_raster <- mask_to_aoi(pred_raster)
    var_raster  <- mask_to_aoi(var_raster)
    log_message(sprintf("  After AOI mask: %d valid cells",
                        sum(!is.na(terra::values(pred_raster)))))
...
if (requireNamespace("viridis", quietly = TRUE)) {
  ...
  kriging_maps_dir <- "outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Maps"
  dir.create(kriging_maps_dir, recursive = TRUE, showWarnings = FALSE)
  ...
  for (stratum_name in unique(cores_standard$stratum)) {
    ...
    tryCatch({
      ...
      # ── 4-panel depth map ──────────────────────────────────────────────────
      map4_path <- file.path(kriging_maps_dir,
      ...
      if (file.exists(total_path)) {
        ...
        total_map_path <- file.path(kriging_maps_dir,
...
if (requireNamespace("viridis", quietly = TRUE)) {
  ...
  for (stratum_name in unique(cores_standard$stratum)) {
    ...
    tryCatch({
      ...
      out_path <- file.path(kriging_maps_dir,
...
# ============================================================================
# KRIGING SPATIAL STATISTICS (equivalent to simple extrapolation outputs)
# ============================================================================
# For each stratum and the whole AOI, compute:
#   - Area (ha)
#   - Mean kriging carbon stock (kg/m² and Mg C/ha)
#   - SE of mean, 95% CI on total stock, VM0033 conservative estimate
# Saved to: outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Tables/
# ============================================================================

log_message("Computing kriging spatial statistics (equivalent to simple extrapolation)...")

kriging_tables_dir <- "outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Tables"
dir.create(kriging_tables_dir, recursive = TRUE, showWarnings = FALSE)

extrap_confidence <- if (exists("EXTRAP_CONFIDENCE")) EXTRAP_CONFIDENCE else 0.95
z_score_ci        <- qnorm(1 - (1 - extrap_confidence) / 2)

kriging_stats_by_stratum <- data.frame()
kriging_stats_aoi        <- data.frame()
...
tryCatch({
  ...
  for (stratum_name in unique(cores_standard$stratum)) {
    ...
    # Load total 0-100 cm kriging stock raster (Mg C/ha)
    total_rast_path <- file.path("outputs/predictions/kriging/aggregated_stocks",
                                 sprintf("stock_total_0-100cm_%s.tif", stratum_token))
    # SE raster: use combined SE at each depth, propagated to total
    # Approximate total SE raster by summing per-depth SE rasters
    se_rast_paths <- lapply(1:nrow(VM0033_DEPTH_INTERVALS), function(i) {
      depth_top    <- VM0033_DEPTH_INTERVALS$depth_top[i]
      depth_bottom <- VM0033_DEPTH_INTERVALS$depth_bottom[i]
      file.path("outputs/predictions/kriging/aggregated_stocks",
                sprintf("stock_se_layer%d_%s_%d-%dcm.tif",
                        i, stratum_token, depth_top, depth_bottom))
    })
    se_rast_paths <- unlist(se_rast_paths[file.exists(unlist(se_rast_paths))])

    if (!file.exists(total_rast_path)) {
      log_message(sprintf("  Skipping spatial stats for %s — total raster not found", stratum_name), "WARNING")
      next
    }

    total_rast <- terra::rast(total_rast_path)
    vals_mgcha <- terra::values(total_rast, na.rm = TRUE)

    # Area: count valid cells × cell area
    cell_area_m2 <- prod(terra::res(total_rast))
    n_valid_cells <- sum(!is.na(terra::values(total_rast)))
    area_m2 <- n_valid_cells * cell_area_m2
    area_ha  <- area_m2 / 10000

    # Mean stock
    mean_MgCha  <- mean(vals_mgcha, na.rm = TRUE)
    sd_MgCha    <- sd(vals_mgcha,   na.rm = TRUE)
    mean_kgm2   <- mean_MgCha / 10

    # SE of mean (spatial SE from kriging raster variability)
    # Use propagated SE raster if available, otherwise fall back to pixel SD / sqrt(n)
    if (length(se_rast_paths) > 0) {
      se_rasters_list <- lapply(se_rast_paths, terra::rast)
      # Propagate SE across depth layers: SE_total = sqrt(sum(SE_i^2))
      # (assumes independence across depth layers)
      se_sq_sum <- Reduce(`+`, lapply(se_rasters_list, function(r) r^2))
      se_total_rast <- sqrt(se_sq_sum)
      se_mean_MgCha <- mean(terra::values(se_total_rast, na.rm = TRUE), na.rm = TRUE)
    } else {
      # Fallback: spatial SE = pixel SD / sqrt(n_cells)
      se_mean_MgCha <- sd_MgCha / sqrt(max(n_valid_cells, 1))
    }

    # Total stock and CI
    total_Mg       <- mean_MgCha * area_ha
    se_total_Mg    <- se_mean_MgCha * area_ha
    ci_half_Mg     <- z_score_ci * se_total_Mg
    lower_Mg       <- total_Mg - ci_half_Mg
    upper_Mg       <- total_Mg + ci_half_Mg
    conservative_Mg <- total_Mg - 1.645 * se_total_Mg   # VM0033 90% one-sided lower
    ...
    row_df <- data.frame(
      ...
      aerial_carbon_kgm2   = mean_kgm2,
      aerial_carbon_MgCha  = mean_MgCha,
      sd_MgCha             = sd_MgCha,
      se_mean_MgCha        = se_mean_MgCha,
      total_carbon_Mg      = total_Mg,
      se_total_Mg          = se_total_Mg,
      ci95_lower_Mg        = lower_Mg,
      ci95_upper_Mg        = upper_Mg,
      conservative_Mg_vm0033 = conservative_Mg,
      confidence_level     = extrap_confidence,
      method               = "Kriging (ordinary, bias-corrected)"
    )

    kriging_stats_by_stratum <- rbind(kriging_stats_by_stratum, row_df)
    log_message(sprintf("  %s: %.1f ha | %.2f Mg C/ha | total %.1f Mg C (95%% CI: %.1f–%.1f)",
                        stratum_name, area_ha, mean_MgCha, total_Mg, lower_Mg, upper_Mg))
  }

  # ── AOI-wide totals ─────────────────────────────────────────────────────────
  if (nrow(kriging_stats_by_stratum) > 0) {
    kriging_stats_aoi <- kriging_stats_by_stratum %>%
      dplyr::summarise(
        n_strata            = dplyr::n(),
        total_area_ha       = sum(area_ha,         na.rm = TRUE),
        total_area_m2       = sum(area_m2,          na.rm = TRUE),
        total_carbon_Mg     = sum(total_carbon_Mg,  na.rm = TRUE),
        se_total_Mg         = sqrt(sum(se_total_Mg^2, na.rm = TRUE)),  # propagated
        confidence_level    = dplyr::first(confidence_level),
        method              = dplyr::first(method)
      ) %>%
      dplyr::mutate(
        ci95_lower_Mg       = total_carbon_Mg - z_score_ci * se_total_Mg,
        ci95_upper_Mg       = total_carbon_Mg + z_score_ci * se_total_Mg,
        conservative_Mg_vm0033 = total_carbon_Mg - 1.645 * se_total_Mg,
        weighted_aerial_MgCha   = total_carbon_Mg / total_area_ha
      )

    # Save tables
    readr::write_csv(kriging_stats_by_stratum,
                     file.path(kriging_tables_dir, "kriging_carbon_stock_by_stratum.csv"))
    readr::write_csv(kriging_stats_aoi,
                     file.path(kriging_tables_dir, "kriging_carbon_stock_aoi_total.csv"))

    log_message(sprintf("Saved kriging spatial statistics tables to %s", kriging_tables_dir))

    # Console summary
    cat("\n========================================\n")
    cat("KRIGING SPATIAL STATISTICS\n")
    cat("========================================\n\n")
    cat(sprintf("%-20s  %-10s  %-12s  %-12s\n",
                "Stratum", "Area (ha)", "Mean (MgC/ha)", "Total (Mg C)"))
    cat(sprintf("%s\n", strrep("-", 58)))
    for (i in seq_len(nrow(kriging_stats_by_stratum))) {
      r <- kriging_stats_by_stratum[i, ]
      cat(sprintf("%-20s  %-10.1f  %-12.2f  %-12.1f\n",
                  r$stratum, r$area_ha, r$aerial_carbon_MgCha, r$total_carbon_Mg))
    }
    cat(sprintf("%s\n", strrep("-", 58)))
    cat(sprintf("%-20s  %-10.1f  %-12.2f  %-12.1f\n",
                "AOI TOTAL", kriging_stats_aoi$total_area_ha,
                kriging_stats_aoi$weighted_aerial_MgCha,
                kriging_stats_aoi$total_carbon_Mg))
    cat(sprintf("\n  95%% CI:   [%.1f – %.1f] Mg C\n",
                kriging_stats_aoi$ci95_lower_Mg,
                kriging_stats_aoi$ci95_upper_Mg))
    cat(sprintf("  VM0033 conservative (90%% lower): %.1f Mg C\n\n",
                kriging_stats_aoi$conservative_Mg_vm0033))
  } else {
    log_message("No kriging rasters available for spatial statistics", "WARNING")
  }

}, error = function(e) {
  log_message(sprintf("Spatial statistics calculation failed: %s", e$message), "WARNING")
})

# ============================================================================
# QUICK-ACCESS COPY: STEP2 KRIGING FOLDER
# ============================================================================

log_message("Copying key Module 04 outputs to outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/...")
...
kriging_copy_dir <- "outputs/Basic_analysis/Step2_Spatial_Basic/Kriging/Maps"
dir.create(kriging_copy_dir, recursive = TRUE, showWarnings = FALSE)

if (length(basic_spatial_sources) == 0) {
  log_message("No kriging spatial outputs found for Step2 copy step", "WARNING")
} else {
  for (src in basic_spatial_sources) {
    dst_name <- build_basic_spatial_name(src)
    dst <- file.path(kriging_copy_dir, dst_name)
    file.copy(src, dst, overwrite = TRUE)
    log_message(sprintf("Copied to Step2/Kriging/Maps: %s", basename(dst)))
  }
}
