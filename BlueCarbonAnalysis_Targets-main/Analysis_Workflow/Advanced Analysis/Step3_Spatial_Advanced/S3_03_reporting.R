...
if (!is.null(method_comparison) && nrow(method_comparison) > 0) {
  ...
  # Build display table from the method-row format written by S3_02
  # Columns available: method, total_area_ha, total_carbon_Mg, se_total_Mg,
  #                    ci95_lower_Mg, ci95_upper_Mg, conservative_Mg
  avail_cols <- names(method_comparison)

  table5_comparison <- method_comparison

  # Rename to human-readable headers, only for columns that exist
  rename_map <- c(
    "Method"                        = "method",
    "Area (ha)"                     = "total_area_ha",
    "Total Carbon (Mg C)"           = "total_carbon_Mg",
    "SE (Mg C)"                     = "se_total_Mg",
    "95% CI Lower (Mg C)"           = "ci95_lower_Mg",
    "95% CI Upper (Mg C)"           = "ci95_upper_Mg",
    "VM0033 Conservative (Mg C)"    = "conservative_Mg"
  )
  rename_map <- rename_map[rename_map %in% avail_cols]

  table5_comparison <- method_comparison %>%
    select(all_of(unname(rename_map))) %>%
    setNames(names(rename_map))
